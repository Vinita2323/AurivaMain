...
import { 
  ...
  Boxes, Users, Tag, Star, Image, Megaphone, 
    ...
    <>
      ...
      <aside className={`fixed top-0 bottom-0 left-0 z-50 w-64 bg-[#0E2A1B] text-[#E8DFC8] border-r border-[#D4AF37]/25 flex flex-col justify-between transition-transform duration-300 ${
            ...
            {links.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  {link.badge && (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#1B3B29] text-[#D4AF37]">
                      {link.badge}
                    </span>
                  )}
                </NavLink>
        ...
        <div className="p-4 border-t border-[#D4AF37]/20 bg-[#0A2014]">
          <Link
            to="/"
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#1B3B29] hover:bg-[#28543B] text-[#D4AF37] border border-[#D4AF37]/30 text-xs font-bold uppercase tracking-wider transition-colors"
          >
